...
└── fetch_all_position_snapshots.py     # 批量获取所有地址的持仓快照⭐
```
...
## 3. fetch_all_position_snapshots.py（批量获取持仓快照）⭐

### **功能说明**
从 `hl_address_list` 表中读取所有活跃地址，逐个获取其**当前持仓状态**。
...
[1/1] 麻吉大哥
   地址: 0x020ca66c30bec2c4fe3861a94e4db4a498a35872
   💰 账户价值: 1342454.157561 USDC
   📊 持仓数量: 1 个
   ✅ 保存成功

======================================================================
执行完成!
结束时间: 2026-04-07 17:29:59
...
| 特性 | 说明 |
|------|------|
| **API 接口** | `clearinghouseState` (获取持仓状态) |
| **数据源** | `hl_address_list` 表（status='active'） |
| **存储表** | `hl_position_snapshots` + `hl_position_details` |
| **去重策略** | `INSERT IGNORE` + `uk_address_time` 唯一约束 |
| **时区处理** | 自动使用北京时间（UTC+8） |
| **执行时间** | 约 5-10 秒（取决于地址数量） |

### **数据结构**

```
一次采集保存：
  ├── 1 条 hl_position_snapshots 记录（账户快照）
  └── N 条 hl_position_details 记录（N = 持仓币种数量）

例如：
  麻吉大哥持有 ETH：
    ├── hl_position_snapshots: 1 条（账户价值 134万）
    └── hl_position_details: 1 条（ETH 持仓明细）
```

### **注意事项**

⚠️ **时间序列数据**：每次采集都会插入新的快照记录，不是覆盖。
